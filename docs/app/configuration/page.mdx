...
The `priority` section defines the options for queues to sort.

| property | type    | example |
| -------- | ------- | ------- |
| quote    | number  | 5       |

- `quote`: the limit of transactions that can be waiting for propagation in fanout stage.

### `priority.queues` section

The `priority.queues` section defines the options to create queues.
