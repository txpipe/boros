...
use pallas::{
    ...
    interop::utxorpc::spec::{
        cardano::{
            script, CostModel, CostModels, ExPrices, ExUnits, ProtocolVersion, RationalNumber, Tx,
            VotingThresholds,
        },
        query::{
            any_chain_params, any_utxo_data, query_service_client::QueryServiceClient,
...
impl U5cDataAdapter for U5cDataAdapterImpl {
    ...
    async fn fetch_utxos(
        ...
        for any_utxo in any_utxos {
            ...
            let parsed_state = any_utxo.parsed_state.unwrap();

            let era = match parsed_state {
                any_utxo_data::ParsedState::Cardano(txo) => {
                    let address = txo.address;
                    let coin = txo.coin;
                    let assets = txo.assets;
                    let datum = txo.datum;
                    let script = txo.script;

                    if !address.is_empty() && coin > 0 && assets.is_empty() && datum.is_none() {
                        match ByronAddress::from_bytes(address.as_ref()) {
                            Ok(_) => Era::Byron,
                            Err(_) => match script.and_then(|s| s.script) {
                                Some(script::Script::Native(native_script))
                                    if native_script.native_script.is_some() =>
                                {
                                    Era::Allegra
                                }
                                Some(_) => Era::Shelley,
    ...
    async fn fetch_pparams(&self, era: Era) -> Result<MultiEraProtocolParameters, anyhow::Error> {
        ...
        let byron_pparams = match pparams.clone() {
        ...
        let shelley_pparams = match pparams.clone() {
            any_chain_params::Params::Cardano(params) => {
                let monetary_expansion =
                    params.monetary_expansion.unwrap_or_else(|| RationalNumber {
                        numerator: 1,
                        denominator: 5,
                    });
                let treasury_expansion =
                    params.treasury_expansion.unwrap_or_else(|| RationalNumber {
                        numerator: 1,
                        denominator: 5,
                    });
                ...
                ShelleyProtParams {
                    ...
                    protocol_version: {
                        let proto = params
                            .protocol_version
                            .unwrap_or_else(|| ProtocolVersion { major: 1, minor: 5 });
        ...
        let alonzo_pparams = match pparams.clone() {
            any_chain_params::Params::Cardano(params) => {
                let monetary_expansion =
                    params.monetary_expansion.unwrap_or_else(|| RationalNumber {
                        numerator: 1,
        ...
        let babbage_pparams = match pparams.clone() {
            any_chain_params::Params::Cardano(params) => {
                let monetary_expansion =
                    params.monetary_expansion.unwrap_or_else(|| RationalNumber {
                        numerator: 1,
        ...
        let conway_pparams = match pparams.clone() {
            any_chain_params::Params::Cardano(params) => {
                let pool_influence = params.pool_influence.unwrap_or_else(|| {
                    RationalNumber {
                        numerator: 1,
                        denominator: 5,
                    }
                });
                let monetary_expansion = params.monetary_expansion.unwrap_or_else(|| {
                    RationalNumber {
                        numerator: 1,
                        denominator: 1,
                    }
                });
                let treasury_expansion = params.treasury_expansion.unwrap_or_else(|| {
                    RationalNumber {
                        numerator: 1,
                        denominator: 1,
                    }
